...
local function getMembersWithRole(self, key, value)
	local str = ""
	return wrap(function()
		for guild in self._guilds:iter() do
			for people in guild.members do
				for role in people.roles do
					if role.name == value then
						str = tostring(people) .. str
					end
				end
			end
		end
...
method('getMembersWithRole', getMembersWithRole, 'name, channel id', "Returns webhooks in that channel.")
