...
local function setNick(self, nick, guild)
	nick = nick or ''
...
local function hasRole(self, key, value)
	local roles = self._parent._roles
	if key == nil and value == nil then return end
	if value == nil then
		value = key
	...
	for _, id in ipairs(self._roles) do
		local role = roles:get(id)
		if role[key] == value then return true end
	end
end

local function hasRoles(self, key, value) -- value must be a table
	local roles = self._parent._roles
	local number, howMany = 0, #value
	if key == nil and value == nil then return end
	if value == nil then
		value = key
		key = roles._key
	...
	for _, id in ipairs(self._roles) do
		if type(value) == "table" then
			for __, tableRole in pairs(value) do
				local role = roles:get(id)
				if role[key] == tableRole then 
					number = number + 1
				end
			end
		end
	end
	if number == howMany then return true end
end
...
method('hasRole', hasRole, 'channel', "Moves a member to a voice channel if they are already in one.")
method('hasRoles', hasRoles, 'channel', "Moves a member to a voice channel if they are already in one.")

cache('Role', getRoleCount, getRole, getRoles, findRole, findRoles, hasRole, hasRoles)
