...
local function getIntegrations(self)
	local client = self._parent
	local success, data = client._api:getGuildIntegrations(self._id)
	if not success then return function() end end
	local i = 1
	return function()
		local v = data[i]
		if v then
			i = i + 1
			return Integration(v, client)
		end
	end
end

local function createIntegration(self, type, id)
	local client = self._parent
	local success, data = client._api:createGuildIntegration(self._id, {
		type = type,
		id = id
	})
	if success then return Integration(data, client) end
end

local function modifyIntegration(self, integration_id, expire_behavior, expire_grace_period, enable_emoticons)
	local client = self._parent
	local success, data = client._api:modifyGuildIntegration(self._id, integration_id {
...
local function deleteIntegration(self, integration_id)
	local client = self._parent
	local success, data = client._api:deleteGuildIntegration(self._id, integration_id)
	if success then return Integration(data, client) end
end

local function syncGuildIntegration(self, integration_id)
	local client = self._parent
	local success, data = client._api:syncGuildIntegration(self._id, integration_id)
	if success then return Integration(data, client) end
end
...
method('getIntegrations', getIntegrations, nil, "Iterator for the guild's Integration(s) (not cached).")
method('createIntegration', createIntegration, "type, id", "Creates a new Integration with the given type (twitch, youtube, etc) and id.")
method('modifyIntegration', modifyIntegration, "integration_id, expire_behavior, expire_grace_period, enable_emoticons", "Modifies Integration.")
method('deleteIntegration', deleteIntegration, "integration_id", "Deletes Integration.")
method('syncGuildIntegration', syncGuildIntegration, "integration_id", "Syncs Integration.")
